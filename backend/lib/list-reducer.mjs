// Materialized-view reduction for the persisted list operation log.
//
// Identity, normalization, and last-write-wins resolution live in the shared
// ../../list-identity.mjs module so the backend view and the UI projection
// (app/listProjection.ts) cannot drift. This file owns only the op-version
// schema and the Map-based reduction over the autobase view.
import {
    DEFAULT_LIST_ID,
    DEFAULT_LIST_TYPE,
    normalizeListId,
...
function upsertItem(list, item, { placement }) {
    ...
    // Updates (preserve placement) honor last-write-wins: a stale edit keeps the
    // newer item and its position. Adds/list snapshots overwrite unconditionally.
