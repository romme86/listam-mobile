...
// Keyword hints for partial/fuzzy fallback. These are intentionally last-resort:
// exact data, aisle modifiers, and collision overrides should win first.
...
const NORMALIZED_ITEM_TO_CATEGORY: Record<string, string> = {}
const NORMALIZED_LANG_ITEM_SETS: Record<SupportedLang, Set<string>> = {} as Record<SupportedLang, Set<string>>
...
const MODIFIER_RULES: Array<{ category: string; terms: string[] }> = [
    {
...
function lookupCategory(raw: string, normalized: string): string | undefined {
    return MULTILANG_ITEM_TO_CATEGORY[raw]
        ?? MULTILANG_ITEM_TO_CATEGORY[normalized]
        ?? NORMALIZED_ITEM_TO_CATEGORY[normalized]
}

function getOverrideCategory(normalized: string): string | undefined {
    return CATEGORY_OVERRIDES.get(normalized)
}

function getModifierCategory(normalized: string): string | undefined {
 ...
 * Exact multilingual data wins first, except for known aisle modifiers and
