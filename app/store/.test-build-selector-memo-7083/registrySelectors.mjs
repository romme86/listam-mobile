...
import { reduceRegistry, REGISTRY_LIST_ID, REGISTRY_LIST_TYPE } from '@listam/domain/list-registry';
import { toNavLibrary } from '@listam/domain/list-nav';
...
const selectDefaultListId = (state) => state.preferences.defaultListId;
const selectBoardEnabled = (state) => state.preferences.boardEnabled;
const selectTodoEnabled = (state) => state.preferences.features.todo;
const selectCurrentListId = (state) => state.lists.selectedListId;
...
const selectBuiltinViews = (state) => state.preferences.builtinViews;
...
export const BUILTIN_SURFACE_TYPES = [DEFAULT_LIST_TYPE, BOARD_LIST_TYPE, TODO_LIST_TYPE];
export function builtinSurfaceNameKey(type) {
    if (isBoardType(type))
        return 'desktop.rail.board';
    if (isTodoType(type))
        return 'desktop.rail.todo';
...
export function isBuiltinSurfaceId(id) {
...
export const selectNavLibrary = createSelector(selectListsState, selectRegistry, selectDefaultListId, selectSurfaceLabels, selectBuiltinGroups, selectBoardEnabled, selectTodoEnabled, selectBuiltinViews, (lists, registry, defaultListId, surfaceLabels, builtinGroups, boardEnabled, todoEnabled, builtinViews) => {
    ...
    const builtinLists = BUILTIN_SURFACE_TYPES.filter((type) => {
        if (isBoardType(type))
            return boardEnabled || hasBoardOnDefault;
        if (isTodoType(type))
            return todoEnabled || hasTodoOnDefault;
        return true;
    }).map((type, i) => {
    ...
    const extraLists = Object.values(lists.listsById)
        .filter((l) => l.id !== REGISTRY_LIST_ID &&
...
export const selectCurrentListBaseKey = createSelector(selectNavLibrary, selectCurrentListId, (lib, currentId) => lib.listsById[currentId]?.baseKey ?? null);
export const selectGroupedLists = createSelector(selectNavLibrary, (lib) => lib.groups.map((group) => ({
    group,
...
export const selectCurrentListView = createSelector(selectNavLibrary, selectCurrentListId, (lib, currentId) => {
    const view = {
        ...DEFAULT_VIEW,
